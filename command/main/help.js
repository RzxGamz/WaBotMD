...
module.exports = {
    ...
    showAdAttribution: true
           ...
           if (timez < "04:00:00") {
               let buffer = await reSize(fs.readFileSync('././lib/media/malam.jpg'), 200, 200)
           } 
           if (timez < "10:00:00") {
               let buffer = await reSize(fs.readFileSync('././lib/media/pagi.jpg'), 200, 200)
           } 
           if (timez < "14:30:00") {
               let buffer = await reSize(fs.readFileSync('././lib/media/siang.jpg'), 200, 200)
           } 
           if (timez < "18:00:00") {
               let buffer = await reSize(fs.readFileSync('././lib/media/sore.jpg'), 200, 200)
           } 
           if (timez < "23:59:00") {
               let buffer = await reSize(fs.readFileSync('././lib/media/malam.jpg'), 200, 200)
           }
