...
function stickerWithExif(file, ext, packInfo, cmd) {
    return new Bluebird(async (res, rej) => {
        ...
        ex.create(packname !== undefined || '' ? packname : "", author !== undefined || '' ? author : "© Rzx Bot", filename);
        await fs.promises.writeFile(`././lib/temp/${filename}.webp`, stickerBuffer);
        run(`webpmux -set exif ././lib/temp/${filename}.exif ././lib/temp/${filename}.webp -o ././lib/temp/${filename}.webp`, async (err) => {
            if (err) rej(err) && await Promise.all([
                fs.unlink(`../temp/${filename}.webp`),
                fs.unlink(`../temp/${filename}.exif`)
            ]);
            setTimeout(() => {
                fs.unlinkSync(`../temp/${filename}.exif`); fs.unlinkSync(`././lib/temp/${filename}.webp`);
            }, 2000);
            res(fs.readFileSync(`././lib/temp/${filename}.webp`));
        })
    });
}
