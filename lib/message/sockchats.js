...
module.exports = (sock, m) => {
	try {
        ...
        const isChats = chats.includes(sender)
        
        // Chats database
        if (isCmd && !isChats) {
        	chats.push({ id: `${sender}`, name: `${pushName}` })
        }
        
        const cmdName = body.slice(prefix.length).trim().split(/ +/).shift().toLowerCase()
        const cmd = this.commands.get(cmdName) || this.commands.find((cmd) => cmd.alias && cmd.alias.includes(cmdName))
